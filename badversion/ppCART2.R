...
#Xcat=c(NULL,0)[1]
ppCART=function(X,y,method='i-classification',NodeRotateFun="RandMatPPO",FunDir=getwd(),paramList=NULL,catLabel=NULL,
                ...
                Levels=NULL,Xscale=c("Min-max","Quantile","No")[1],TreeRandRotate=FALSE)
{
  ...
  paramList = ppRF:::defaults(paramList,p,catLabel=catLabel)
  #D=paramList$D
  ...
  while(!is.null(nodeXIndx[[currentNode]])){
    ...
    if(!NodeRotateFun%in%c("RandMatPPO","RandMatPPR","ppOpt")){
      #paramList = ppRF:::defaults(paramList,x=X[nodeXIndx[[currentNode]],],y=y[nodeXIndx[[currentNode]]],
      #                            method=method,catLabel=catLabel)
      if(!NodeRotateFun%in%ls("package:ppRF")){
        paramList$x =X[nodeXIndx[[currentNode]],];paramList$y = y[nodeXIndx[[currentNode]]];
    ...
    if(NodeRotateFun=="ppOpt"){
      sparseM=ppRF:::ppOpt(x=X[nodeXIndx[[currentNode]],],y=y[nodeXIndx[[currentNode]]],q = 1L,
              ppMethod =  ifelse(is.null(paramList$ppMethod),"LDA",paramList$ppMethod),
              weight = TRUE,r = 1L,lambda = 0.1,energy = 0,cooling = 0.9,TOL = 1e-04,maxiter = 1000L)
      sparseM=sparseM$projMat
    }
    
    if(NodeRotateFun=="RandMatPPO"){
      sparseM=RandMatPPO(x=X[nodeXIndx[[currentNode]],],y=y[nodeXIndx[[currentNode]]],numProj=paramList$numProj,
               dimProj=paramList$dimProj,catLabel = paramList$catLabel,
               ppMethod =  ifelse(is.null(paramList$ppMethod),"NNet",paramList$ppMethod),method=method)
    }
    
    if(NodeRotateFun=="RandMatPPR"){
      sparseM=RandMatPPR(x=X[nodeXIndx[[currentNode]],],y=y[nodeXIndx[[currentNode]]],numProj=paramList$numProj,
                 catLabel = paramList$catLabel,method=method)
    }
  ...
  ppTree=list()
  ppTree$call=match.call()
  ppTree$method=method
  ppTree$Levels=Levels
  ppTree$NodeRotateFun=NodeRotateFun
  ppTree$data=list(n=n,p=p,varName=colnames(X),Xscale=Xscale,minCol=minCol,maxminCol=maxminCol,Xcat=Xcat,catLabel=catLabel,
  ...
  ppTree$stuction=list(nodeRotaMat = nodeRotaMat,nodeCutValue =nodeCutValue[1:(currentNode-1)],nodeCutIndex =nodeCutIndex[1:(currentNode-1)],
                       childNode = childNode[1:(currentNode-1)],nodeLabel = nodeLabel[1:(currentNode-1)],
                       nodeNumLabel = nodeNumLabel[1:(currentNode-1)],nodeDepth=nodeDepth[1:(currentNode-1)])
  return(ppTree)
}