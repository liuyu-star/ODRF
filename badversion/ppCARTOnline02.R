...
#Xcat=c(NULL,0)[1]
ppCARTOnline02=function(ppTree,X=NULL,y=NULL)
{
  #ppTree=ppTree0
  #X=Xnew
  #y=ynew
  ...
  ppTree=ppTree[-2]
  ppTreeVar=c("method","NodeRotateFun",names(ppTree$data),names(ppTree$tree),names(ppTree$stuction))
  ...
  if(all(nodeCutValue==0)){
    ...
    Nodes = .Call('_ppRF_ppCARTNode', PACKAGE = 'ppRF',X, nodeRotaMat, nodeCutValue, childNode)
  ...
  while(!is.null(nodeXIndx[[currentNode]])){
    ...
    if(currentNode<=numNode0){
      ...
      if(freeNode<=numNode0){
        ...
        childNode0=c(childNode0[idx],0,0,childNode0[-idx])
  ...
  ppTree$stuction=list(nodeRotaMat = nodeRotaMat,nodeCutValue =nodeCutValue[1:(currentNode-1)],childNode = childNode[1:(currentNode-1)],
                       nodeLabel = nodeLabel[1:(currentNode-1)],nodeDepth=nodeDepth[1:(currentNode-1)])
  return(ppTree)
}
