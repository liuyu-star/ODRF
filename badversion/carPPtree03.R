...
carPPtree=function(X,y,method='c',FUN="RandMatPPR",paramList=NULL,catMap=NULL,MaxLayer=Inf,
                   minleaf=ifelse(method=='r',5,1),weights=1,Levels=if(method=='r')NULL else levels(as.factor(y)))
{
  ...
  while(nodeflags[current_node] == 1){
    ...
    if(funName=="RandMatPPR"){
      sparseM <- RandMatPPR(x=X[currentXIndx,],y=y[currentXIndx],method=method,catMap=paramList$catMap)
    }else{
