...
\SweaveOpts{engine=R, eps=FALSE, keep.source = TRUE, concordance=FALSE}%TRUE}%
...
%
<<classification and regression>>=
index <- seq(floor(nrow(seeds_train) / 2))
forest1 <- ODRF(varieties_of_wheat ~ ., seeds_train[index, ],
  split = "entropy", parallel = FALSE)
pred <- predict(forest1, seeds_test[, -8])
e.forest.1 <- mean(pred != seeds_test[, 8])
forest.online <- online(forest1, seeds_train[-index, -8], seeds_train[-index, 8])
pred <- predict(forest.online, seeds_test[, -8])
e.forest.online <- mean(pred != seeds_test[, 8])
forest.prune <- prune(forest1, seeds_train[-index, -8],
  seeds_train[-index, 8], useOOB = FALSE)
pred <- predict(forest.prune, seeds_test[, -8])
e.forest.prune <- mean(pred != seeds_test[, 8])
print(c(forest1 = e.forest.1, forest.online = e.forest.online,
...
tree.online <- online(tree1, bodyfat_train[-index, -1], bodyfat_train[-index, 1])
pred <- predict(tree.online, bodyfat_test[, -1])
e.tree.online <- mean((pred - bodyfat_test[, 1])^2)
tree.prune <- prune(tree1, bodyfat_train[-index, -1], bodyfat_train[-index, 1])
pred <- predict(tree.prune, bodyfat_test[, -1])
e.tree.prune <- mean((pred - bodyfat_test[, 1])^2)
print(c(tree1 = e.tree.1, tree.online = e.tree.online, tree.prune = e.tree.prune))
@
...
<<RotMat>>=
data(seeds, package = "ODRF")
(PP <- PPO(seeds[, 1:7], seeds[, 8], model = "LDA", split = 'gini'))
RotMat <- RotMatPPO(seeds[, 1:7], seeds[, 8],model = "Log",
  split = 'gini')
head(RotMat)
data(body_fat, package = "ODRF")
(PP <- PPO(body_fat[, 2:15], body_fat[, 1], model = "Log", split = 'mse'))
RotMat <- RotMatPPO(seeds[, 1:7], seeds[, 8],model = "PPR",
  split = 'gini')
head(RotMat)
set.seed(22)
...
set.seed(230108)
data(breast_cancer)
#train <- sample(1:569, 500)
#train_data <- data.frame(breast_cancer[train, -1])
#test_data <- data.frame(breast_cancer[-train, -1])
tree <- ODT(diagnosis ~ ., breast_cancer[, -1], split = "entropy",#"gini",
            paramList = list(model = "Log", numProj = 1))
print(tree)
@
...
\begin{figure}[t!]
\centering
<<plot,echo=FALSE,fig=TRUE,height=4,width=10>>=
plot(tree)
@
