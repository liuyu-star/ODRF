set.seed(1)
X=X;y=as.numeric(y);y1=as.numeric(y1);method='c';FUN="RandMatPPR";paramList=NULL;catMap=NULL;MaxLayer=Inf;
...
while(nodeFlags[current_node] == 1){
  ...
  if(funName=="RandMatPPR"){
    ...
    rotaX=X[nodeXIndx[[current_node]],,drop = FALSE] %*% t(RandMat);
    
    ppCoef <- RandMatPPR(x=rotaX,y=y[nodeXIndx[[current_node]]],method=method)
    RandMat=t(RandMat)%*%ppCoef
    ppVar=which(RandMat!=0)
    sparseM=cbind(ppVar,rep(1,length(ppVar)),RandMat[ppVar])
    
    #rotaX=rotaX %*% ppCoef;
    rotaX=X[nodeXIndx[[current_node]],,drop = FALSE] %*% RandMat
    rm(RandMat)
  }else{
    sparseM <- do.call(FUN, paramList)
