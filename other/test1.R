...
while(nodeFlags[current_node] == 1){
  ...
  if(funName=="RandMatPPR"){
    ppVar=sample(1:p,ceiling(p/3))
    ppCoef <- RandMatPPR(x=X[nodeXIndx[[current_node]],ppVar],y=y[nodeXIndx[[current_node]]],
                          method=method,catMap=paramList$catMap)
    sparseM=cbind(ppVar,rep(1,length(ppVar)),ppCoef)
    
    rotaX=X[nodeXIndx[[current_node]],ppVar] %*% ppCoef;
  }else{
    sparseM <- do.call(FUN, paramList)
