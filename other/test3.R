set.seed(1)
X=X;y=as.numeric(y);y1=as.numeric(y1);method='r';FUN="RandMatPPR";paramList=NULL;catMap=NULL;MaxLayer=Inf;
...
if(is.null(paramList)){
  paramList <- ppRF1:::defaults(ncolX = p, paramList = paramList, cat.map = catMap)
}
...
while(nodeFlags[current_node] == 1){
  ...
  if(funName=="RandMatPPR"){
    ...
    bestCut =ppRF1:::best_cut_node(method,rotaX0,y[nodeXIndx[[current_node]]],Wcd,minleaf,maxLabel);
    orderVar=order(bestCut$MinVal)
    #ppCoef <- RandMatPPR(x=rotaX0,y=y[nodeXIndx[[current_node]]],method=method)
    
    sparseM=NULL;
    rotaX=rotaX0;
    for (j in 1:ncol(rotaX0)) {
      RandVar=orderVar[1:j]
      ppCoef <- RandMatPPR(x=rotaX0[,RandVar,drop = FALSE],y=y[nodeXIndx[[current_node]]],method=method)
  ...
  sparseM=sparseM[sparseM[,2]==bestCut$BestCutVar,,drop = FALSE]
