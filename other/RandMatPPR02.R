...
RandMatPPR02 <- function(x, y, d=NULL,catMap = NULL,method='c',...) {
  p = ncol(x)
  n=length(y)
...
if(1==2){
  RandMatPPR02 <- function(x, y, d=NULL,catMap = NULL,method='c',...) {
    ...
    if(is.null(d)){
      q =min(ceiling(length(y)^0.4),ceiling(ncol(x)*2/3))
      d=min(p,max(5, ceiling(p/q)))
    }
    ...
    if(n>10){
      ...
      for (ni in unique(sparseM1[,2L])) {
        ...
        if (length(Ii) > 1L) {
          theta.i <- try(ppr(Xi, Yi, nterms = 1, bass=1)$alpha, silent = TRUE)# sm.method="spline",sm.method="gcvspline"
