...
test_that("no warning if data.frame has two classes", {
  ...
  expect_silent(ODT(Species ~ ., data = iris, type = "i-classification"))
})

test_that("Error if subset is 0", {
  expect_error(ODT(Species ~ ., data = iris, type = "g-classification", subset = c(0)))
})
...
test_that("Error if argument 'X' dimension equal to 1", {
  ...
  X <- matrix(runif(n))
  y <- rnorm(n)
  expect_error(ODT(X, y, type = "g-classification"))
})
...
test_that("no warning if data.frame has Missing value, NA or NaN", {
  ...
  expect_silent(ODT(y ~ ., data = dat, type = "regression"))
})

test_that("regression splitting not working on classification data", {
  expect_error(ODT(Species ~ ., iris, type = "regression"))
})
...
test_that("holdout mode holding out data with 0 weight", {
  ...
  expect_silent(ODT(Species ~ ., data = iris, type = "i-classification", weights = weights))
})
...
test_that("Split points are at (A+B)/2 for numeric features, i-classification splitting", {
  ...
  tree <- ODT(y ~ ., data = dat, type = "i-classification", NodeRotateFun = "RotMatRF")
...
test_that("Split points are at (A+B)/2 for numeric features, g-classification splitting", {
  dat <- data.frame(y = rbinom(100, 1, .5), matrix(rbinom(5 * 100, 1, .5), 100, 5))
  tree <- ODT(y ~ ., data = dat, type = "g-classification", NodeRotateFun = "RotMatRF")
  split_points <- tree$structure$nodeCutValue
  nsp <- which(split_points != 0)
  expect_equal(split_points[nsp], rep(0.5, length(nsp)))
})
...
test_that("Split points are at (A+B)/2 for numeric features, regression", {
  ...
  tree <- ODT(y ~ ., data = dat, type = "regression", NodeRotateFun = "RotMatRF")
  split_points <- tree$structure$nodeCutValue
  nsp <- which(split_points != 0)
  expect_equal(split_points[nsp], rep(0.5, length(nsp)))
})
...
set.seed(1)
tree1 <- ODT(Species ~ ., data = iris, type = "g-classification", NodeRotateFun = "RotMatRand")
pred1 <- predict(tree1, Xnew = iris[!ind, -5])

set.seed(1)
tree2 <- ODT(Species ~ ., data = iris, type = "g-classification", NodeRotateFun = "RotMatRand")
pred2 <- predict(tree2, Xnew = iris[!ind, -5])

set.seed(2)
tree3 <- ODT(Species ~ ., data = iris, type = "g-classification", NodeRotateFun = "RotMatRand")
