...
forest <- ODRF(Species ~ ., data = iris, type = "i-classification", parallel = FALSE)
...
test_that("Error if data is class matrix, classification", {
  expect_error(ODRF(Species ~ ., data = dat, type = "i-classification", parallel = FALSE))
})
...
test_that("Error if y is a factor type and type = 'regression'", {
  ...
  X <- matrix(runif(5 * n), n, 5)
  y <- as.factor(rbinom(n, 1, 0.5))
  expect_error(ODRF(X = X, y = y, type = "regression", parallel = FALSE))
})
