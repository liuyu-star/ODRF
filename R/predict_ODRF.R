...
#' \item \code{prob}: matrix of class probabilities (one column for each class and one row for each input). If \code{ppForest$split} is \code{mse}, a vector of tree weights is returned.
...
predict.ODRF <- function(object, Xnew, type = "response", weight.tree = FALSE, ...) {
  ...
  if(!is.null(ppForest$data$catLabel)&&(sum(ppForest$data$Xcat)>0)){
    pp=pp-length(unlist(ppForest$data$catLabel))+length(ppTree$data$Xcat)
  }
  if(ncol(Xnew)!=pp){
